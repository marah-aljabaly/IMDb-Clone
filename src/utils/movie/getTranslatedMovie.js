...
const getMovieTrans = async (movie_id) => {
  const options = {
    method: "GET",
    url:`https://api.themoviedb.org/3/movie/${movie_id}?language=ar-SA`,
    headers: {
  ...
  try {
    ...
    console.error("Failed to fetch Arabic Movie:", error);
    // return navigate('/error'); // يمكنك إرجاع مصفوفة فارغة أو التعامل مع الخطأ كما تشاء
  }
};

export default getMovieTrans; // تصدير الدالة مباشرة
